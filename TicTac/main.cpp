...
void main()
{
	...
	const int n = 9;
	char field[n] = {};
	//for (int i = 0; i < n; i++)field[i] = i + 49; //49- ASKII-код символа 1
	PrintField(field, n, 'X');
	do
	{
		InitField(field, n, '0');
		cout << "Еще разочек? Да - anykey, Нет - Eskape" << endl;
...
void PrintField(char field[], const int n, char player)
{
	...
	// точнее любую программу, котору можно запустить из командной строки
...
void Move(char field[], const int n, char player)
{
	...
	do
	{
		key = _getch();
		if (key == 27)return;
		if (key < '1' || key>'9')cout << "\aВы нажали неправильно\n";
		// \a (alert/alarm) - 
...
void Check(char field[], const int n, char player)
{
	bool game_over = false;
		...
		bool draw = true; //изначально предпологаем что у нас ничья
